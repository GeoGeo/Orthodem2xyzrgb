...
'''
parser = argparse.ArgumentParser(description='Create XYZRGB point cloud from orthomosaic and DSM')
parser.add_argument('-m', action='store', dest='mosaic', help='Filename of Orthomosaic')
parser.add_argument('-d', action='store', dest='dsm', help='Filename of DEM/DSM')
parser.add_argument('-o', action='store', dest='outfilename', help='Filename for xyzrgb')
parser.add_argument('-r', action="store", dest="resolution", type=int, default=32)
results = parser.parse_args()

tick = time.time()

rdr = RastersToXYZRGB(results.mosaic,
                      results.dsm,
                      results.resolution,
                      nodata = [0, -10000])


FILEOUT = results.outfilename

fo = open(FILEOUT,"w")
print 
print "\nOrthodem2XYZRGB"
print "by Steven Kay, GeoGeo\n"
print "Raster size\t\t%d x %d" % (rdr.red.cols, rdr.red.rows)
print "Pixel size\t\t%2.4fm x %2.4fm" % (rdr.red.xpixelsize, rdr.red.ypixelsize)
print "Sample every\t\t%d pixels (%2.4fm x %2.4fm)" % (rdr.red.tilesize, rdr.red.xpixelsize*results.resolution, rdr.red.ypixelsize*results.resolution)
print "DSM file\t\t%s" % rdr.filenameDSM
print "Mosaic file\t\t%s" % rdr.filenameOrtho
print "Output to\t\t%s" % FILEOUT
print 
fo.write("X,Y,Z,R,G,B\n")
cnt = 0
ruler = "|....:...."*10
for progress, point in rdr.getPointsAndProgress():
    if len(point) > 0:
        x,y,z,r,g,b = point
        line = "%2.4f,%2.4f,%2.4f,%d,%d,%d\n" % (x, y, z, r, g, b)
        fo.write(line)
    else:
        pass
    if progress==0: 
        progress = 1
    print "%s [%01d%%] \r" % (ruler[0:progress], progress-1),
    cnt += 1
fo.close()
print "\n\nFinished"
print "Took %2.4f secs" % ( time.time()-tick )
print "Output %d rows to %s" % ( cnt, FILEOUT )
print "Dropped %d NODATA rows" % rdr.dropped
print
'''